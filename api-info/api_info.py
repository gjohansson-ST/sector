...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
            ...
            print(datetime.now())
            async with session.get(API_URL + "/Panel/getFullSystem",
            ...
            print(datetime.now())
            async with session.get(API_URL + "/Panel/GetPanelStatus?panelId={}".format(PANELID),
                headers=message_headers) as response:
                if response.status != 200:
                    #_LOGGER.debug("Sector: Failed to get Full system: %d", response.status)
                    #raise PlatformNotReady
            ...
            print(datetime.now())
            async with session.get(API_URL + "/Panel/GetTemperatures?panelId={}".format(PANELID),
                headers=message_headers) as response:
                if response.status != 200:
                    #_LOGGER.debug("Sector: Failed to get Full system: %d", response.status)
                    #raise PlatformNotReady
            ...
            print(datetime.now())
            async with session.get(API_URL + "/Panel/GetLockStatus?panelId={}".format(PANELID),
                headers=message_headers) as response:
                if response.status != 200:
                    #_LOGGER.debug("Sector: Failed to get Full system: %d", response.status)
                    #raise PlatformNotReady
            ...
            print(datetime.now())
            async with session.get(API_URL + "/Panel/GetLogs?panelId={}".format(PANELID),
                headers=message_headers) as response:
                if response.status != 200:
                    #_LOGGER.debug("Sector: Failed to get Full system: %d", response.status)
                    #raise PlatformNotReady
