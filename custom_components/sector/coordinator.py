...
class SectorDataUpdateCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    async def triggerlock(
        self, lock_serial: str, code: str, command: str, panel_id: str
    ) -> None:
        ...
        try:
            if command == "unlock":
                await self._request(API_URL + "/api/Panel/Unlock", json_data=message_json)
                self.data[panel_id]["lock"][lock_serial]["status"] = "unlock"
            if command == "lock":
                await self._request(API_URL + "/api/Panel/Lock", json_data=message_json)
                self.data[panel_id]["lock"][lock_serial]["status"] = "lock"
        except (UpdateFailed, ConfigEntryAuthFailed) as error:
            raise HomeAssistantError(
                f"Could not lock {lock_serial} on error {str(error)}"
            ) from error
    ...
    async def triggerswitch(self, identity: str, command: str, panel_id: str) -> None:
        ...
        try:
            if command == "on":
                await self._request(
                    f"{API_URL}/api/Panel/TurnOnSmartplug?switchId={identity}&panelId={panel_id}",
                    json_data=message_json,
                )
                self.data[panel_id]["switch"][identity]["status"] = "On"
            if command == "off":
                await self._request(
                    f"{API_URL}/api/Panel/TurnOffSmartplug?switchId={identity}&panelId={panel_id}",
                    json_data=message_json,
                )
    ...
    async def triggeralarm(self, command: str, code: str, panel_id: str) -> None:
        ...
        try:
            if command == "full":
                await self._request(API_URL + "/api/Panel/Arm", json_data=message_json)
                self.data[panel_id]["alarmstatus"] = 3
            if command == "partial":
                await self._request(
                    API_URL + "/api/Panel/PartialArm", json_data=message_json
                )
                self.data[panel_id]["alarmstatus"] = 2
            if command == "disarm":
                await self._request(API_URL + "/api/Panel/Disarm", json_data=message_json)
    ...
    async def async_first_refresh(self) -> dict[str, Any]:
        ...
        response_panellist = await self._request(API_URL + "/api/account/GetPanelList")
        ...
        for panel in response_panellist:
            ...
            response_getpanel = await self._request(
                API_URL + f"/api/Panel/GetPanel?panelId={panel_id}"
            )
        ...
        response_getuser = await self._request(API_URL + "/api/Login/GetUser")
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        ...
        for key, panel in data.items():
            ...
            response_get_status = await self._request(
                API_URL + f"/api/Panel/GetPanelStatus?panelId={panel_id}"
            )
            ...
            if data[key].get("temp") and self._update_sensors:
                ...
                response_temp = await self._request(
                    API_URL + f"/api/Panel/GetTemperatures?panelId={panel_id}"
                )
            ...
            if data[key].get("lock"):
                ...
                response_lock = await self._request(
                    API_URL + f"/api/Panel/GetLockStatus?panelId={panel_id}"
                )
            ...
            if data[key].get("switch"):
                ...
                response_switch = await self._request(
                    API_URL + f"/api/Panel/GetSmartplugStatus?panelId={panel_id}"
                )
            ...
            response_logs = await self._request(
                API_URL + f"/api/Panel/GetLogs?panelId={panel_id}"
            )
