...
class SectorDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    async def _async_update_data(self):
        ...
        try:
            ...
            return {"devices": devices, "panel_status": panel_status, "logs": self._event_logs}
    ...
    def _process_devices(self, api_data):
    ...
    def _process_locks(self, locks_data: list, devices: dict) -> None:
        ...
        for lock in locks_data:
            ...
            _LOGGER.debug("Processed lock with serial_no %s: %s", serial_no, devices[serial_no])

    def _process_category_devices(self, category_name: str, category_data: dict, devices: dict) -> None:
                ...
                for place in section.get("Places", []):
                    for component in place.get("Components", []):
                        ...
                        if serial_no:
                            ...
                            _LOGGER.warning("Component missing SerialNo/Serial: %s", component)
        else:
            _LOGGER.debug("Category %s does not contain Sections.", category_name)

    def _add_sensor_if_present(self, sensors: dict, component: dict, sensor_key: str, source_keys: Any, transform=None):
    ...
    def _process_event_logs(self, logs, devices):
        ...
        for log_entry in logs:
            ...
            if not serial_no:
                _LOGGER.debug("Log entry for unknown lock name '%s', skipping: %s", lock_name, log_entry)
                continue
            ...
            _LOGGER.debug(
                ...
                lock_name, serial_no, event_type, timestamp, user or "unknown user", channel or "unknown channel"
            )
