...
async def async_setup_entry(
    ...
    for device in devices.values():
        ...
        if "temperature" in sensors:
            entities.append(SectorAlarmTemperatureSensor(coordinator, serial_no, device_info))
            _LOGGER.debug("Added temperature sensor for device %s", serial_no)

        if "humidity" in sensors:
            entities.append(SectorAlarmHumiditySensor(coordinator, serial_no, device_info))
...
class SectorAlarmSensor(SectorAlarmBaseEntity, SensorEntity):
    ...
    def __init__(self, coordinator, serial_no, device_info, sensor_type, description):
