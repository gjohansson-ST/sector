...
async def async_setup_entry(
    ...
    for lock in locks_data:
        ...
        if serial_no:
            ...
            LOGGER.warning(f"Lock missing SerialNo: {lock}")

    if entities:
        async_add_entities(entities)
    else:
        LOGGER.debug("No lock entities to add.")
...
class SectorAlarmLock(CoordinatorEntity, LockEntity):
    ...
    def __init__(
        ...
        self._serial_no = lock_data.get("SerialNo") or lock_data.get("Serial")
        self._attr_unique_id = f"{self._serial_no}_lock"
        self._attr_name = lock_data.get("Label", "Sector Lock")
        LOGGER.debug(f"Initialized lock with unique_id: {self._attr_unique_id}")
    ...
    def is_locked(self):
        ...
        for lock in locks:
            if (lock.get("SerialNo") or lock.get("Serial")) == self._serial_no:
